...
export function VendorsPage({ notify }: { notify: Notify }) {
  ...
  return <>
    <WorkspaceIntro eyebrow="Companies / Relationship records" title="Vendors" description="Keep ownership, current reviews, contacts, and evidence boundaries together without turning vendor claims into campus policy." actions={<DitherAction onClick={addVendor}><Plus size={14} /> Add vendor</DitherAction>} />
    <div className="workspace-split">
      <section className="workspace-panel workspace-records"><div className="workspace-toolbar"><SearchBox value={query} onChange={setQuery} placeholder="Search vendors" /><span>{filtered.length} records</span></div><div className="workspace-column-labels"><span>Vendor</span><span>Owner</span><span>Status</span></div><div className="workspace-record-list">{filtered.map((vendor) => <button key={vendor.id} className={selected.id === vendor.id ? "selected" : ""} onClick={() => setSelectedId(vendor.id)}><span className="workspace-record-identity"><PersonAvatar name={vendor.name} /><span><strong>{vendor.name}</strong><small>{vendor.domain} · {vendor.activeRuns} active reviews</small></span></span><span className="workspace-record-owner"><PersonAvatar name={vendor.owner} size={26} /><span><strong>{vendor.owner}</strong><small>{vendor.ownerRole}</small></span></span><span><Pill>{vendor.status}</Pill><small>{vendor.route}</small></span><ChevronRight size={15} /></button>)}</div></section>
      <aside className="workspace-panel workspace-detail"><div className="workspace-detail-head"><PersonAvatar name={selected.name} size={44} /><div><p className="workspace-eyebrow">Vendor record</p><h2>{selected.name}</h2><span>{selected.domain}</span></div><Pill>{selected.route}</Pill></div><div className="workspace-detail-actions"><DitherAction onClick={() => notify(`Message draft opened for ${selected.owner}.`)}><Mail size={14} /> Email owner</DitherAction><PlainAction onClick={() => notify(`${selected.name} record opened locally.`)}><ExternalLink size={14} /> Open record</PlainAction></div><dl className="workspace-definition-grid"><div><dt>Internal owner</dt><dd>{selected.owner}</dd></div><div><dt>Role</dt><dd>{selected.ownerRole}</dd></div><div><dt>Contacts</dt><dd>{selected.contacts}</dd></div><div><dt>Last review</dt><dd>{selected.lastReview}</dd></div></dl><section className="workspace-detail-section"><p className="workspace-eyebrow">Current context</p><p>{selected.note}</p></section><section className="workspace-detail-section"><p className="workspace-eyebrow">Relationship map</p><div className="relationship-line"><PersonAvatar name={selected.owner} size={28} /><strong>{selected.owner}</strong><ArrowRight size={14} /><Building2 size={15} /><strong>{selected.name}</strong></div></section><div className="workspace-boundary"><FolderLock size={16} /><span><strong>Vendor-scoped evidence</strong> stays with {selected.name}. It cannot modify deterministic campus policy.</span></div></aside>
    </div>
  </>;
}
...
export function ContactsPage({ notify }: { notify: Notify }) {
  ...
  return <>
    <WorkspaceIntro eyebrow="People / Relationship records" title="Contacts" description="Connect vendor contacts to the internal staff member accountable for each relationship." actions={<DitherAction onClick={addContact}><UserPlus size={14} /> Add contact</DitherAction>} />
    <div className="workspace-split"><section className="workspace-panel workspace-records"><div className="workspace-toolbar"><SearchBox value={query} onChange={setQuery} placeholder="Search contacts" /><span>{filtered.length} people</span></div><div className="workspace-record-list workspace-contact-list">{filtered.map((contact) => <button key={contact.id} className={selected.id === contact.id ? "selected" : ""} onClick={() => setSelectedId(contact.id)}><span className="workspace-record-identity"><PersonAvatar name={contact.name} /><span><strong>{contact.name}</strong><small>{contact.role}</small></span></span><span><strong>{contact.vendor}</strong><small>{contact.linkedStaff}</small></span><Pill>{contact.status}</Pill><ChevronRight size={15} /></button>)}</div></section><aside className="workspace-panel workspace-detail"><div className="workspace-detail-head"><PersonAvatar name={selected.name} size={44} /><div><p className="workspace-eyebrow">{selected.type} contact</p><h2>{selected.name}</h2><span>{selected.role}</span></div><Pill>{selected.status}</Pill></div><div className="workspace-detail-actions"><DitherAction onClick={() => notify(`Email draft opened for ${selected.name}.`)}><Mail size={14} /> Email contact</DitherAction><PlainAction onClick={() => notify(`${selected.name} attached to ${selected.vendor}.`)}><Link2 size={14} /> Attach</PlainAction></div><dl className="workspace-definition-grid"><div><dt>Email</dt><dd>{selected.email}</dd></div><div><dt>Contact type</dt><dd>{selected.type}</dd></div><div><dt>Vendor / portfolio</dt><dd>{selected.vendor}</dd></div><div><dt>Linked staff</dt><dd>{selected.linkedStaff}</dd></div></dl><section className="workspace-detail-section"><p className="workspace-eyebrow">Relationship map</p><div className="relationship-line"><PersonAvatar name={selected.name} size={28} /><strong>{selected.name}</strong><ArrowRight size={14} /><Building2 size={15} /><strong>{selected.vendor}</strong></div><p>{selected.type === "Vendor" ? `${selected.linkedStaff} owns the internal relationship.` : `${selected.name} can be assigned as the reporting person on vendor records.`}</p></section></aside></div>
  </>;
}
...
export function TasksPage({ notify }: { notify: Notify }) {
  ...
  return <><WorkspaceIntro eyebrow="Human work / Follow-ups" title="Tasks" description="Reviewer-owned follow-ups remain visible and separate from deterministic policy results." actions={<DitherAction onClick={() => notify("New task draft created.")}><Plus size={14} /> New task</DitherAction>} /><div className="workspace-split"><section className="workspace-panel workspace-records"><div className="workspace-toolbar"><SearchBox value={query} onChange={setQuery} placeholder="Search tasks" /><span>{filtered.length} tasks</span></div><div className="task-list">{filtered.map((task) => <button key={task.id} className={selected.id === task.id ? "selected" : ""} onClick={() => setSelectedId(task.id)}><span className="task-check">{task.status === "Complete" ? <Check size={13} /> : <ClipboardCheck size={13} />}</span><span><strong>{task.title}</strong><small>{task.id} · {task.related}</small></span><span><Pill>{task.status}</Pill><small>{task.due}</small></span></button>)}</div></section><aside className="workspace-panel workspace-detail"><p className="workspace-eyebrow">Task record / {selected.id}</p><h2>{selected.title}</h2><p>Linked to {selected.related}</p><div className="workspace-detail-actions"><DitherAction disabled={selected.status === "Complete"} onClick={complete}><CheckCircle2 size={14} /> Mark complete</DitherAction><PlainAction onClick={() => notify(`${selected.related} opened from task.`)}><Link2 size={14} /> Open review</PlainAction></div><dl className="workspace-definition-grid"><div><dt>Assignee</dt><dd>{selected.assignee}</dd></div><div><dt>Due</dt><dd>{selected.due}</dd></div><div><dt>Priority</dt><dd>{selected.priority}</dd></div><div><dt>Status</dt><dd>{selected.status}</dd></div></dl><div className="workspace-boundary"><ShieldCheck size={16} /><span>Completing a task documents work. It cannot change the policy route or approve a request.</span></div></aside></div></>;
}
...
export function NotesPage({ notify }: { notify: Notify }) {
  ...
  return <><WorkspaceIntro eyebrow="Shared context / Handoffs" title="Notes" description="Capture durable context without copying unsupported claims into policy results." actions={<DitherAction onClick={add}><Plus size={14} /> New note</DitherAction>} /><div className="notes-workspace"><section className="workspace-panel notes-list">{notes.map((note) => <button key={note.id} className={selected.id === note.id ? "selected" : ""} onClick={() => select(note.id)}><div><Pill>{note.tag}</Pill><span>{note.updated}</span></div><strong>{note.title}</strong><p>{note.body}</p><small>{note.author}</small></button>)}</section><section className="workspace-panel note-editor"><div><p className="workspace-eyebrow">{selected.id} / {selected.tag}</p><h2>{selected.title}</h2><span>{selected.author} · {selected.updated}</span></div><textarea value={draft} onChange={(event) => setDraft(event.target.value)} aria-label="Review note text" /><div className="note-editor-actions"><span><BookOpen size={14} />Notes are context, not policy.</span><DitherAction onClick={save}><Check size={14} /> Save note</DitherAction></div></section></div></>;
}
...
export function ChatPage({ notify }: { notify: Notify }) {
  ...
  return <><WorkspaceIntro eyebrow="Reviewer assistant / Read-only" title="Chat" description="Ask grounded questions across the current workspace without giving the assistant policy or approval authority." /><div className="chat-workspace"><section className="workspace-panel chat-main"><header><span className="chat-spark"><Sparkles size={17} /></span><span><strong>CSUB review copilot</strong><small>Grounded in this local workspace</small></span><Pill>Read-only analysis</Pill></header><div className="chat-messages">{messages.map((item, index) => <article className={`chat-message chat-${item.role}`} key={`${item.role}-${index}`}><span>{item.role === "assistant" ? <Sparkles size={14} /> : <PersonAvatar name="Alex Reviewer" size={26} />}</span><div><small>{item.role === "assistant" ? "Review copilot" : "You"}</small><p>{item.text}</p></div></article>)}</div><div className="chat-suggestions"><button onClick={() => setMessage("Summarize today’s review queue")}>Summarize queue</button><button onClick={() => setMessage("Show missing evidence")}>Show evidence gaps</button><button onClick={() => setMessage("Explain the LabArchives route")}>Explain a route</button></div><div className="chat-composer"><input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") send(); }} placeholder="Ask about a review, vendor, or source…" aria-label="Message review copilot" /><DitherAction onClick={send}>Send <ArrowRight size={14} /></DitherAction></div><footer>Chat can explain and summarize. It cannot approve, change policy, or write to ServiceNow.</footer></section><aside className="workspace-panel chat-context"><p className="workspace-eyebrow">Current scope</p><h2>Workspace context</h2><dl><div><dt>Review runs</dt><dd>5 loaded</dd></div><div><dt>Vendor evidence</dt><dd>Case-scoped</dd></div><div><dt>Policy sources</dt><dd>Separate scope</dd></div><div><dt>Human decisions</dt><dd>Required</dd></div></dl><div className="workspace-boundary"><ShieldCheck size={16} /><span>Retrieved content is untrusted and cannot override system boundaries.</span></div></aside></div></>;
}

export function SettingsPage({ notify }: { notify: Notify }) {
  const tabs = ["Workspace", "Review controls", "Members", "Integrations", "Notifications"] as const;
  ...
  return <><WorkspaceIntro eyebrow="Administration / Local prototype" title="Settings" description="Preview workspace controls without changing live AWS, Box, or ServiceNow configuration." /><div className="settings-workspace"><nav className="workspace-panel settings-nav">{tabs.map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}><Settings2 size={14} />{item}</button>)}</nav><section className="workspace-panel settings-main"><header><div><p className="workspace-eyebrow">{tab}</p><h2>{tab} settings</h2><p>Changes stay in this browser for the prototype.</p></div><Pill>Prototype</Pill></header>{tab === "Workspace" && <div className="settings-form"><label>Workspace name<input defaultValue="CSUB Technology Review" /></label><label>Workspace description<textarea defaultValue="Human-reviewed vendor management and technology review operations." /></label><label>Default evidence scope<select defaultValue="Vendor and case scoped"><option>Vendor and case scoped</option><option>Campus policy only</option></select></label></div>}{tab === "Review controls" && <div className="settings-toggles">{([['matches','Require human confirmation for non-exact matches','Fuzzy and semantic candidates always pause.'],['writeback','Require a second write-back confirmation','Mock connector updates remain two-step.'],['policy','Keep policy route immutable','Specialist findings cannot change routing.'],['evidence','Enforce retrieval boundaries','Policy, case, and vendor sources remain separate.']] as const).map(([key,title,detail]) => <label key={key}><span><strong>{title}</strong><small>{detail}</small></span><input type="checkbox" checked={controls[key]} onChange={() => toggle(key)} /></label>)}</div>}{tab === "Members" && <div className="member-list">{["Alex Reviewer", "Jordan Lee", "Maya Patel"].map((member) => <article key={member}><PersonAvatar name={member} /><span><strong>{member}</strong><small>{member === "Alex Reviewer" ? "Admin · Human reviewer" : "Reviewer"}</small></span><Pill>Active</Pill></article>)}</div>}{tab === "Integrations" && <div className="integration-grid">{[["Mock ServiceNow","Configured preview and simulated write-back.",ShieldCheck],["Box source corpus","Read-only source manifest for prototype evidence.",FolderLock],["Review assistant","Local grounded analysis surface.",Sparkles]].map(([name,detail,Icon]) => { const IntegrationIcon = Icon as typeof ShieldCheck; return <article key={String(name)}><span><IntegrationIcon size={17} /></span><strong>{String(name)}</strong><p>{String(detail)}</p><Pill>Connected</Pill></article>; })}</div>}{tab === "Notifications" && <div className="settings-toggles"><label><span><strong>Human-action alerts</strong><small>Show assignments and evidence holds in the inbox.</small></span><input type="checkbox" defaultChecked /></label><label><span><strong>Evidence freshness warnings</strong><small>Surface stale or mismatched sources.</small></span><input type="checkbox" defaultChecked /></label></div>}<footer><span>Local prototype configuration</span><DitherAction onClick={() => notify(`${tab} settings saved locally.`)}><Check size={14} /> Save changes</DitherAction></footer></section></div></>;
}
...
export function DocumentationPage({ notify }: { notify: Notify }) {
  ...
  return <><WorkspaceIntro eyebrow="Operating guide / Prototype v0.1" title="Documentation" description="The practical guide for reviewers using this local technology-review workspace." /><div className="docs-workspace"><nav className="workspace-panel docs-list">{sections.map((item, index) => <button key={item.title} className={selected === index ? "active" : ""} onClick={() => setSelected(index)}><span><item.icon size={16} /></span><span><strong>{item.title}</strong><small>{item.detail}</small></span><ChevronRight size={15} /></button>)}</nav><article className="workspace-panel docs-article"><header><span>[ CSUB / REVIEW GUIDE ]</span><em>0{selected + 1}</em></header><div className="docs-rule" /><p className="workspace-eyebrow">Core operating guide</p><h2>{section.title}</h2><p className="docs-lede">{section.detail}</p><section><CheckCircle2 size={17} /><span><strong>Assistant boundary</strong><p>The assistant may extract, compare, summarize, and draft from approved clauses. Its output remains reviewable.</p></span></section><section><ShieldCheck size={17} /><span><strong>Human and deterministic boundary</strong><p>People confirm non-exact matches and decisions. Versioned rules calculate the route. External write-back remains simulated.</p></span></section><footer><span>Source-linked · Human-reviewed</span><DitherAction onClick={() => notify("Review operations support request drafted.")}><LifeBuoy size={14} /> Contact operations</DitherAction></footer></article></div></>;
}
