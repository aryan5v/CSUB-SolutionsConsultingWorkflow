...
export function WorkflowsPage({ view, navigate, notify }: { view: WorkflowView; navigate: (page: WorkflowView) => void; notify: Notify }) {
  ...
  return <><WorkspaceIntro eyebrow="Automation / Local definitions" title={view === "workflows" ? "Workflows" : view === "workflow-runs" ? "Workflow runs" : "Workflow versions"} description="Compose bounded review automation without allowing a workflow or model to establish policy or approve a request." />
    <nav className="workflow-tabs" aria-label="Workflow sections"><button className={view === "workflows" ? "active" : ""} onClick={() => navigate("workflows")} aria-current={view === "workflows" ? "page" : undefined}><Workflow size={14} aria-hidden="true" />Definitions</button><button className={view === "workflow-runs" ? "active" : ""} onClick={() => navigate("workflow-runs")} aria-current={view === "workflow-runs" ? "page" : undefined}><CircleDotDashed size={14} aria-hidden="true" />Runs</button><button className={view === "workflow-versions" ? "active" : ""} onClick={() => navigate("workflow-versions")} aria-current={view === "workflow-versions" ? "page" : undefined}><History size={14} aria-hidden="true" />Versions</button></nav>
