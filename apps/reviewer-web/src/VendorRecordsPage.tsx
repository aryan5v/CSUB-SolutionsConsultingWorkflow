  ...
  return <>
    ...
    {loading ? <section className="workspace-panel record-loading" role="status">Loading vendor records…</section> : <div className="vendor-record-layout">
      <section className="workspace-panel vendor-record-builder" aria-label="Vendor intake golden path">
        <div className="record-step"><span aria-hidden="true">01</span><div><h2>Vendor</h2><label>Select vendor<select value={selectedVendorId} onChange={(event) => chooseVendor(event.target.value)} aria-label="Select vendor"><option value="">Select a vendor</option>{vendors.map((vendor) => <option key={vendor.vendor_id} value={vendor.vendor_id}>{vendor.name}{vendor.review_status && vendor.review_status !== "no_cases" ? ` · ${vendor.review_status.replace(/_/g, " ")}` : ""}</option>)}</select></label><form className="record-inline-form" onSubmit={createVendor}><input name="name" required placeholder="New vendor name" aria-label="New vendor name" /><input name="domain" placeholder="official.example" aria-label="Official domain" /><button type="submit" disabled={busy}><Plus size={13} aria-hidden="true" />Create</button></form>{selectedVendor?.reviewed_products?.length ? <ul className="record-status-list" aria-label="Reviewed products">{selectedVendor.reviewed_products.map((item) => <li key={`${item.product_id}-${item.case_id}`}><strong>{item.product_name}</strong><small>{item.review_status.replace(/_/g, " ")} · {item.case_id}</small></li>)}</ul> : null}</div></div>
