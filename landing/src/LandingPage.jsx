/**
 * from Paper
 * https://app.paper.design/file/01KWYYM4JH5ESNFXC20VP55C90/1-0/1-0
 * Responsive fluid layout; same Vetted visual system.
 */

function PixelLogo({ size = 30, className, style, opacity }) {
  return (
    <svg
      className={className}
      width={size}
...
function Machine({ className, style }) {
  return (
    <svg
      className={className}
      viewBox="0 0 320 300"
...
function IsoTile({ variant, className, style }) {
  const face = (
    <>
      {variant === 'doc' && (
        <>
      ...
      <div className="vp-inner">
        ...
        <section className="vp-hero" id="top">
          ...
          <div className="vp-hero-copy land-fade-up">
            ...
            <p className="vp-hero-lead land-fade-up-delay">
              Requesters submit proposed software and vendor evidence. Vetted looks up the approved-software export, extracts facts with bounded specialists, applies deterministic policy rules, and drafts a cited packet — then a reviewer makes the final call.
            </p>
            ...
            <div className="vp-checks land-fade-up-delay-2">
              {['AI extracts evidence', 'Rules calculate the route', 'Humans approve'].map((label) => (
          ...
          <div className="vp-scene land-fade-up-delay-2" aria-hidden="true">
            <div className="vp-scene-canvas">
              ...
              <div className="vp-chip vp-chip-b land-float-delayed">
                ...
                Flag only · still needs human review
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="vp-trusted" id="product">
          ...
          <div className="vp-trusted-row">
            {['UI', 'APIs', 'Storage', 'LangGraph', 'Rules', 'Review', 'ServiceNow mock', 'AWS'].map((brand) => (
              <div key={brand} className="vp-trusted-item">
        ...
        <section className="vp-block" id="workflow">
          <div className="vp-header">
            <p className="vp-eyebrow">EVALUATION LOGIC</p>
            <h2 className="vp-h2">You give docs. Three layers finish the review.</h2>
          </div>
            ...
            {[
              {
                ...
                body: 'After intake and approved-software lookup, security and accessibility specialists read vendor and institutional materials in parallel. Evidence and official-vendor research fill supported gaps. A citation checker rejects unsupported claims.',
                tile: 'doc',
              },
              {
                n: '02',
                title: 'Deterministic rules decide the route',
                body: 'Normal code — not the model — applies versioned flowchart and decision-tree rules. It returns risk route, required evidence, conflicts, and citations. Models may explain; they cannot change the calculated result.',
                tile: 'spark',
              },
              {
                n: '03',
                title: 'Human makes the final decision',
                body: 'Reviewers see the merged findings and drafted low- or medium-risk packet, confirm fuzzy matches, then approve, reject, or request more information. Write-back is a second explicit confirmation to a mock ServiceNow connector.',
                tile: 'check',
              },
            ].map((step) => (
        ...
        <section className="vp-block" id="trust">
          ...
          <div className="vp-header">
            <p className="vp-eyebrow">IMPORTANT FLAGS</p>
            ...
            {[
              {
                ...
                body: 'If the product is already on the export, that is disclosed as a candidate. If it is not — ✗ Not on approved software list — that is a flag for review, not an automatic reject. Fuzzy or semantic matches still need a reviewer to confirm.',
                iconBg: '#F7DC6F',
                icon: (
              ...
              {
                title: 'Specialists + rules',
                body: 'Specialists extract and summarize (security, accessibility, evidence). The business-rules engine calculates risk route and required documents from partner flowcharts and decision trees. AI drafts; rules enforce policy.',
                iconBg: '#3178C6',
                icon: (
                  <>
              ...
              {
                title: 'Reviewer is the doctor',
                body: 'Low-risk recommendation or editable medium-risk TAAP/security packet. High-risk, contradictory, unsupported, or incomplete cases escalate. No approval, signature, or ServiceNow write without a recorded human decision.',
                iconBg: '#333333',
                icon: (
                  <>
              ...
              <article key={card.title} className="vp-feature">
                <div className="vp-feature-icon" style={{ background: card.iconBg }}>
                  <svg width="20" height="20" viewBox="0 0 20 20" aria-hidden="true">{card.icon}</svg>
                </div>
        ...
        <section className="vp-stats" aria-label="Key facts">
          {[
            { value: 'Extract', label: 'specialists pull facts from policies, HECVAT, SOC 2, VPAT, and more' },
            { value: 'Route', label: 'deterministic rules set risk path — low, medium, or escalate' },
            { value: 'Decide', label: 'reviewer approves, rejects, or asks for more information' },
          ].map((stat) => (
        ...
        <section className="vp-cta-wrap" id="demo">
          <div className="vp-cta">
            ...
            <div className="vp-cta-copy">
              <h2 className="vp-cta-title">From request to mock ServiceNow — with a human in the middle.</h2>
              <p className="vp-cta-lead">
                Frontend → APIs → storage → LangGraph specialists → business rules → reviewer UI → MockServiceNowConnector, on AWS.
              </p>
              <a className="vp-btn vp-btn-primary" href="#signup">
                Sign up
                <ArrowIcon dark />
              </a>
            </div>
        ...
        <footer className="vp-footer">
          <div className="vp-footer-brand">
            <PixelLogo size={24} />
            Vetted · AI extracts · rules route · humans decide
          </div>
          <div className="vp-footer-links">
            <a href="#login">Login</a>
            <a href="#signup">Sign up</a>
            <span className="vp-footer-copy">© 2026</span>
          </div>
        </footer>
      </div>
    </div>
